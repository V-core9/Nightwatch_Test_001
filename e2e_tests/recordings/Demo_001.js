const verbose = true;

describe("Demo_001", function () {
  it("tests Demo_001", function (browser) {
    browser
      .windowRect({ width: 1858, height: 1009 })
      .navigateTo(
        "https://www.google.com/search?q=nightwatch+export+test+extension&rlz=1C1GCEA_enRS1004RS1005&oq=&aqs=chrome.0.69i59i450l2.218397361j0j15&sourceid=chrome&ie=UTF-8"
      )
      .clickExtended(
        browser,
        "#rso > div:nth-of-type(2) > div > div > div:nth-of-type(1) h3",
        { verbose }
      )
      .clickExtended(browser, "button.vwo-survey-minimize path", { verbose })
      .clickExtended(browser, "ul:nth-of-type(4) a", { verbose })
      .clickExtended(browser, "div.e-f-Ln > a:nth-of-type(2)", { verbose })
      .clickExtended(browser, "div.F-k span:nth-of-type(2)", { verbose })
      .end();
  });
});
